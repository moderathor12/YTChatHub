...
import { ttsService } from '../../tts-service'; // TTS servisini import et
...
export default function DashboardPage() {
  ...
  const toggleTTS = useCallback(() => {
    ...
    ttsService.toggleTTS(newState);
  }, [ttsEnabled]);

  // Yeni mesajları seslendir
  useEffect(() => {
    ...
    messages.forEach(message => {
      if (!previousMessageIdsRef.current.has(message.id)) {
        // Bu yeni bir mesaj, seslendir
        const textToSpeak = message.text || 'Mesaj içeriği yok';
        const username = message.author;
        
        // Super Chat ise özel mesaj
        if (message.superChat) {
          ttsService.speak(
            `Süper sohbet: ${message.superChat.amount} ${message.superChat.currency}. ${textToSpeak}`,
            username
          );
        } else if (message.membershipGift || message.membershipGiftPurchase) {
          ttsService.speak(`Yeni üye katıldı: ${textToSpeak}`, username);
        } else {
          ttsService.speak(textToSpeak, username);
        }
      }
    });
        ...
        <>
          <header className="dashboard__tabs">
            ...
            <button 
              className="btn-tts" 
              onClick={toggleTTS}
              title={ttsEnabled ? 'Seslendirmeyi Kapat' : 'Seslendirmeyi Aç'}
            >
              {ttsEnabled ? '🔊 TTS Açık' : '🔇 TTS Kapalı'}
            </button>
